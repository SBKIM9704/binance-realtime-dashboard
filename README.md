...
```
  ...
  ● ready  backfill 163.6s

  BTCUSDT    259,362 rows  ██▇▇▇▇▇▇▇▇▇▆▆▆▆▆▆▅▅▅▄▃▃▃▃▃▃▂▁▁▁▁▁▁▂▂▂▂▂▂▂▁▁▁▁▁▁▁
           63,234.01  -0.10%   gaps 1743/1743 ✓
  ETHUSDT    259,280 rows  ██████████▇▆▆▆▆▆▆▆▆▆▆▄▃▄▄▄▄▂▁▁▁▁▂▃▃▃▃▃▃▂▁▁▁▁▁▁▁▁
           1,870.38  -0.10%   gaps 1742/1742 ✓
```

> **최초 실행**은 `BACKFILL_DAYS`(기본 3일)치 1초봉을 채운 뒤 실시간 수집을 시작하며, 동시에
> 히스토리 티어를 백그라운드로 받습니다. 기본 설정 실측(2종목 합계): **약 165초 · 838 REST 요청 ·
> 129MB**. 티어 백필은 실시간 수집을 막지 않습니다.
>
> 대부분은 1초봉 백필입니다 — 3일치가 심볼당 259,200봉(260 페이지)이라 전체 요청의 62%를
> 차지합니다. `BACKFILL_DAYS`를 줄이면 최초 실행이 그만큼 짧아집니다.
...
| 티어 | 보관 | 용도 | 실측 |
|------|------|------|------|
| `1s` (수집 인터벌) | 7일 | 운영 감시 · 짧은 기간 차트 | 3일치 심볼당 259,200행 (260요청) |
...
| 항목 | 결과 |
|------|------|
| 최초 백필 | `first-run` 동작 확인 — 3일치 1초봉 심볼당 259,200봉, 838 REST 요청, 163.6초, 129MB |
