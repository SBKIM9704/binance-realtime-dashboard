...
이 대시보드는 투자 화면이 아니라 운영 화면입니다. 따라서 선정 기준은 *"수집 품질을 가장 먼저
확인할 수 있는가"*였고, 운영자 시선 흐름에 맞춰 **System → Pipeline → Market** 3계층으로
구성했습니다.
...
### 한 가지 짚어둘 판단 — 24h 집계의 데이터 출처

24h 집계를 수집 인터벌(1초봉)에서 계산했더니, 수집 공백 때문에 **거래량에 최대 60% 가까운 오차**가
발생했습니다. 집계에 필요한 것은 정밀도가 아니라 완결성이므로, 24h 지표는 해당 구간을 덮으면서
조밀한 티어에서 계산하도록 변경했습니다. (오차 −35%/−60% → **−0.03%/−0.01%**)

> 데이터 흐름도 · 티어 구조 · 데이터 품질 원칙 · 장애 시나리오별 대시보드 반응 · 표시 단위 설계는
> 👉 **[`docs/dashboard-metrics.md`](docs/dashboard-metrics.md)** 에 정리했습니다.
