...
기본 설정(`KLINE_INTERVAL=1s`, `BACKFILL_DAYS=3`, `HISTORY_TIERS=1m:30,1h:0`, 2심볼) 실측입니다.

### 최초 실행 (1회성)

| 항목 | 봉 수 (심볼당) | 요청 수 | weight |
|------|---------------|---------|--------|
| 1초봉 백필 (3일치) | 259,200 | 260 × 2 = **520** | ≈ 1,040 |
| 1분봉 티어 (30일) | 43,200 | 44 × 2 = **88** | ≈ 176 |
| 1시간봉 티어 (상장일부터) | 78,405 | 79 × 2 = **158** | ≈ 316 |
| 초기 reconcile · 티어 top-up | — | **≈ 72** | ≈ 144 |
| **합계** | | **838건** | **≈ 1,676** |

163.6초에 걸쳐 분산되므로 순간 사용량은 분당 약 **615 weight**, 한도의 **10% 수준**입니다.
`REST_THROTTLE_MS`(기본 250ms)가 페이지 요청 사이를 벌려 버스트를 막습니다.

`BACKFILL_DAYS`가 전체의 62%를 차지하므로, 최초 실행 비용을 줄이려면 이 값을 낮추는 것이
가장 효과적입니다.
